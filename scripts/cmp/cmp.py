...
0x247 (arrival-only DLC8, zeros suffice) is the frame the DI monitors to clear cmpMIA,
NOT the ODIN-documented CMP_info 0x363 (not in the PMR RX filter set). compact.json
mislabels 0x247 "DAS_autopilotDebug".
"""
...
class Cmp(Node):
    ...
    def _frames_2022(self) -> list[SimFrame]:
        # 2022.45.15 adds 0x2A7 — the config-selected alternate cmpMIA frame (a config bit picks
        # 0x247 vs 0x2A7). DLC8, arrival-only (no E2E), zeros suffice.
