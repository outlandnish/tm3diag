#!/usr/bin/env python3
"""PMR bl/bu device verifyCRC algorithm (UDS verifyCRC, RC 0x0201).

The device compares this CRC against the host-supplied expected value. NOT zlib.crc32 — a
forward-poly (0x04C11DB7), MSB-first, table-driven CRC-32 that processes the flash region one
16-bit WORD at a time, folding the word's HIGH byte then LOW byte (big-endian within each word).

Table (256 x uint32) first two entries 0x00000000, 0x04C11DB7 confirm poly 0x04C11DB7
(NOT zlib's reflected 0xEDB88320).

Inner loop (per word, crc = running 32-bit crc):
    idx  = (word>>8) ^ (crc>>24)          # fold high byte of word
    crc  = ((crc<<8) ^ table[idx]) & 0xFFFFFFFF
    idx  = ((crc>>24) ^ word) & 0xff      # fold low byte of word
    crc  = ((crc<<8) ^ table[idx]) & 0xFFFFFFFF

init seed and final xor are configurable; default init=0, xorout=0.

Byte order: the C28x is 16-bit-word addressed and the .bhx flash bytes are byte-swapped relative
to the loadable image (c28x_loadimg.py --mode swap). The device CRCs the words as they sit in flash
(swapped order), so feed this CRC the swapped image bytes packed as 16-bit words. crc32_bytes_be()
packs each consecutive byte pair as one big-endian 16-bit word.
"""

# --- CRC-32 table (256 x uint32), two halves ---
...
if __name__ == "__main__":
    # --- self-test 1: table == freshly generated forward-poly table ---
    ...
    # --- self-test 2: sample reference values ---
